...
namespace TutorialGame.Source.Engine.Input.CustomKeyboard
{
    internal class clsKey
    {
        ...
        public clsKey(string KEY, int STATE)
        {
            key = KEY;
            state = STATE;
            MakePrint(key);
        ...
        public void MakePrint(string KEY)
        {
            ...
            if (KEY == "Space") { tempStr = " "; }
            if (KEY == "OemCloseBrackets") { tempStr = "]"; display = tempStr; }
            if (KEY == "OemOpenBrackets") { tempStr = "["; display = tempStr; }
            if (KEY == "OemMinus") { tempStr = "-"; display = tempStr; }
            if (KEY == "OemPeriod" || KEY == "Decimal") { tempStr = "."; }
            if (KEY == "D1" || KEY == "D2" || KEY == "D3" || KEY == "D4" ||
